...
const obtenerCorrelativo = async () => {
    try {
        ...
        const [resApi] = await connection.execute(`SELECT MAX(pedcorint) as maximo FROM tbl_pedidos`);
...
const insertPedido = async (db, data, productos) => {
  //inserta pedido en base pedidos_api
    const sqlHeader = `
        INSERT INTO pedidos_api.tbl_pedidos 
        (
   ...
   //inserta detalle del pedido en base pedidos_api
    if (productos && productos.length > 0) {
        const sqlDetalle = `
            INSERT INTO pedidos_api.tbl_pedidos_detalle (pedido_id, prod_id, cantidad, empresa, pedcorint)
            VALUES (?, ?, ?, ?, ?)
        `;
